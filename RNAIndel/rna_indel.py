...
def get_args():
    ...
    parser.add_argument('-o', '--output-vcf', metavar='FILE', required=True, type=check_output, help='output vcf file')
    parser.add_argument('-f', '--fasta', metavar='FILE', required=True, help='reference genome (GRCh38) FASTA file')
    ...
    parser.add_argument('-r', '--refgene', metavar='FILE', default=REFGENE, help='refgene coding exon database')
    parser.add_argument('-d', '--dbsnp', metavar='FILE', default=DBSNP,
                        help='indels on dbSNP database in vcf format')
    parser.add_argument('-l', '--clinvar', metavar='FILE', default=CLINVAR, help='ClinVar database')
