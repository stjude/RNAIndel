...
def rescue_by_nearest(row, fasta, bam, search_window):
    ...
    pos_move = [-int(i) if int(i) == i else int(i+1) \
                for i in np.array(range(serch_window))/2]
    i = 0
    idl_found = None
    while i < len(pos_move) and not idl_found:
        idl_found = extract_indels(pos+pos_move[i], fasta, bam, chr, idl_type)
        i += 1
...
def extract_indels(pos, fasta, bam, chr, idl_type, **kwargs):
