...
import indel_curator_dev as ic
from functools import partial
from indel_processor_protein import acc_len_dict
from indel_sequence_dev import SequenceWithIndel

mrna = re.compile(r'NM_[0-9]+')

def main(df, fasta, refgene):    
...
def generate_indel(row, fasta):
    ...
    return ic.curate_indel_in_genome(fasta, chr, pos, idl_type, idl_seq)
...
def check_equivalence(row, df):
    ...
    Args:
       row (pandas.Series): A Series object column indexed 'indel_obj'
       df (pandas.DataFrame)
    Returns:
       str: see example below

    Example:
     indel objects are stored in dataframe 'indel_obj' column.
     ...
     This scripts find equivalent indel objects for each object.
...
def equivalence_id_dict():
    ...
    Args:
       None
          The hard-coded file name 'equivalent_indels.txt'
